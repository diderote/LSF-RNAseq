...
def submission_prepend(submission=None, source=None, conda='RNAseq', module_list=[]):
    '''
...
def send_job(command_list, job_name, job_log_folder, q, mem, log_file, project, cores=1, submit=False):
    ...
    cmd = '''\n'''.join(['#!/bin/bash',
                         ...
                         f'#BSUB -o {job_log_folder}ID_{rand_id}_JOB_{job_name}.out',
                         f'#BSUB -e {job_log_folder}ID_{rand_id}_JOB_{job_name}.err',
                         '#BSUB -W 120:00',
                         f'#BSUB -n {cores}',
                         f'#BSUB -q {q}',
                         f'#BSUB -P {project}',
...
def final_qc(exp):
    try:
        ...
        command_list = [submission_prepend(module_list=['python']), f'cd {exp.scratch}', 'multiqc *']
